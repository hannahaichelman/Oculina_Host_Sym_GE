...
sym_counts_summary = sym_counts_summary %>%
  rownames_to_column() %>%
  dplyr::rename("SampleID1" = "rowname") %>%
  dplyr::rename("SymCounts" = "colSums.countDataSym.")
...
Check to see if we set up our contrast correctly. We should have the treatment condition first and the control second in the log2 fold change (MLE) output. 
```{r, cache=TRUE}
head(results)
```
