...
# this is the final merged dataframe that we will use to try the new DESeq model
...
```{r,cache=TRUE}
expDesign_old = read.csv("~/Dropbox/BU/Host_Buffering/FinalGithub/Oculina_Host_Sym_GE/08_tables/ExpDesign_orthos_noculture.csv")
...
write.csv(stats.per.sample, file = "~/Dropbox/BU/Host_Buffering/FinalGithub/Oculina_Host_Sym_GE/08_tables/oculina_orthos_oldcounts_summary_table.csv", quote = FALSE)
...
keep.dds.old = rowSums(counts(dds.old) >= 2) >= 43 # at least 43 samples (80%) with a count of 2 or higher
dds.corr.old = dds.old[keep.dds.old,]
...
# These results are out of 185 nonzero read counts - original sco mapping
