...
```{r,cache=TRUE, echo=FALSE}
countDataSym_tomerge <- read.table("/Users/hannahaichelman/Dropbox/BU/Host_Buffering/MPCC_2018/Sym_analyses/CountsFiles/Oculina_Counts_newref_sym.txt", row.names = NULL)
colnames(countDataSym_tomerge)[1] = "contig"

countDataCulture_tomerge <- read.table("/Users/hannahaichelman/Dropbox/BU/Host_Buffering/MPCC_2018/Sym_analyses/CountsFiles/B_psygmophilum_counts.txt", row.names = NULL)
...
# $type
#                                  diff         lwr         upr     p adj
# culture_hot-culture_cold -0.059999658 -0.09816988 -0.02182944 0.0003195
# host_cold-culture_cold   -0.057166253 -0.09533647 -0.01899603 0.0006984
# host_hot-culture_cold     0.012994448 -0.02517577  0.05116467 0.8177370
# host_cold-culture_hot     0.002833406 -0.03533682  0.04100363 0.9975372
# host_hot-culture_hot      0.072994107  0.03482389  0.11116433 0.0000056
# host_hot-host_cold        0.070160701  0.03199048  0.10833092 0.0000144


lm1 = lm(abs_log2foldchange~type, data=res_combined)
summary(lm1)

# try splitting up into up- and down-regulated genes
res_combined_pos = res_combined %>%
  filter(log2FoldChange > 0)

aov_pos = aov(abs_log2foldchange~type, data=res_combined_pos)
summary(aov_pos)
TukeyHSD(aov_pos)

res_combined_neg = res_combined %>%
  filter(log2FoldChange < 0)

aov_neg = aov(abs_log2foldchange~type, data=res_combined_neg)
summary(aov_neg)
TukeyHSD(aov_neg)
...
# what is the relationship of this line:
lm_heat <- lm(log2FoldChange.inhost ~ log2FoldChange.inculture, hot_res_merged_anno) 
summary(lm_heat)
my_equation_heat <- paste("y =",        # Extract equation of model
                     coef(lm_heat)[[1]],
                     "+",
                     coef(lm_heat)[[2]],
                     "* x")
my_equation_heat   # Print equation of model

# [1] "y = -0.0532470628552914 + 0.608385970135619 * x"
...
# what is the relationship of this line:
lm_cold <- lm(log2FoldChange.inhost ~ log2FoldChange.inculture, cold_res_merged_anno) 
summary(lm_cold)
my_equation_cold <- paste("y =",        # Extract equation of model
                     coef(lm_cold)[[1]],
                     "+",
                     coef(lm_cold)[[2]],
                     "* x")
my_equation_cold   # Print equation of mode```
e4# [1] "y = -0.0194476898444039 + 0.336978917947459 * x"
